...
- **Staffel-Konfigurationen werden direkt übernommen:** `angebot_bauen` liest bei einer Rezeptur-Anfrage **alle** vom Kunden angefragten Staffelzeilen (`portal_anfrage_pos`: Anzahl/Füllmenge je Verpackung + Menge VPE) und legt je Zeile eine eigene **Gruppe** (A/B/C …) im Angebot an (`angebot_gruppe_anhaengen()` + `angebot_rezeptur_zeilen()`). Der Wunsch-Verpackungstyp wird best effort auf einen konkreten Behälter aufgelöst (`passende_behaelter_fuer()` + `verpackung_passt_zu_typ()`), sonst wählt das Team ihn im Editor. So muss nicht jede Staffel von Hand nachgetragen werden.
...
Neben jedem Angebot in der Liste stehen der **Status** (farbig, mit Umlauten – „bestätigt", nicht der Rohwert aus der Datenbank) und ein **⇩** für das Angebots-PDF (`?p=angebot_pdf&id=…`, neuer Tab).
