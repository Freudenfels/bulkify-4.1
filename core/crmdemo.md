...
- **Layout**: `cd_head/cd_shell_start/cd_shell_ende` (eigenes Menü, Rollen-Chips, Sprach-Chips, Theme).
  Das Menü in `cd_shell_start()` ist eine flache Liste. Die einzelnen KI-Funktionen stehen NICHT im
  Menü, sondern **als Button auf der passenden Seite** (COA/Spec-Reader auf dem Rohstoff-Katalog,
  Produktentwickler auf dem Rezeptur-Katalog, KI-Rohstoff-Chat auf der Konversation) und gebündelt
  unter dem einen Menüpunkt **„KI-Werkzeuge"** (Modul `m=ki` = Hub mit Karten zu jeder KI).
