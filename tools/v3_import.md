...
## Stufe 6 (Anfragen + Angebote)
Jede v3-`produktanfrage` wird zu einer Kunden-Anfrage **`portal_anfrage`** (Portal „Meine Anfragen", idempotent über `v3_id`) – mit Angebot Status „beantwortet" (Kunde sieht „Angebot erhalten"), reine offene Anfrage ohne Preis „neu". So sieht der Kunde – wie in v3 – seine Anfragen UND die Angebote. Ein **`angebot`** entsteht nur, wenn Preis/Bestätigung vorliegt; es wird per `angebot.anfrage_id` mit der Anfrage verknüpft.
...
**Stück je Packung je Staffel:** `produktanfrage_staffel.menge_pro_vpe` wird nach `angebot_staffel.stueck` übernommen (Kapseln/Tabletten je Packung; bei Pulver = Portionen). Die Angebotskarte zeigt daraus das Packungs-Label: Kapseln/Tabletten als Anzahl (z. B. „180 Kapseln"), Pulver/Granulat als **Gramm-Packung** = Portionen × Tagesdosis (Rezeptursumme), z. B. „250 g" – für den Kunden greifbarer.
...
**Waisen aus v3:** Manche `produktanfrage` zeigen auf ein `rezept_id`, das in v3 gelöscht wurde (die `rezepte`-Tabelle beginnt bei id 6, hat Lücken). Solche Angebote werden importiert; der Produktname wird – wo möglich – aus dem Angebots-PDF-Dateinamen gerettet (`v3_name_aus_datei()`, z. B. „mylab_report_Magnesium Komplex.pdf" → „Magnesium Komplex"), sonst kundenneutral „Produkt". Der technische Grund („Rezeptur in v3 gelöscht / Name unbekannt") steht NUR in der internen Angebots-Notiz, nicht in der Position (die ist auch im Kundenportal sichtbar). Die Kunden-Angebotstabelle zeigt den Positionsnamen, wenn kein Produkt verknüpft ist (statt „–"). Das ist kein Import-Fehler; alle real existierenden Rezepte werden übernommen.
